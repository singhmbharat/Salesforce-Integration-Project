...
async function callSalesforceAPI(instanceUrl,accessToken,urlPath,method ,data){
...
functions.http('helloHttp',async(req,res)=>{
  ...
  try{
    ...
    console.log('its still working..1..');
   ...
   //const newquoteObjId = newquoteObj.record.id;

    console.log('sourcequote record =>'+sourcequoteObj.record.Id);
    console.log('sourcequote lineitems =>'+sourcequoteObj.lineItems);
    const pricebookId = sourcequoteObj.record.pricebookId;
    const currency = sourcequoteObj.record.currency;
    ...
    for(const index in productList){
        console.log('prodId=>'+productList[index]);
    }

    const contextObj = { pricebookId : pricebookId , currencyCode : currency};
    const contextbody = {context : JSON.stringify(contextObj)};
    ...
    for(const index in productList){
       var prod = await callSalesforceAPI(
        instanceUrl,
        accessToken,
        `/services/apexrest/SBQQ/ServiceRouter?loader=SBQQ.ProductAPI.ProductLoader&uid=${productList[index]}`,
        'PATCH',
        {"context" : "{\"pricebookId\": \"01s2t000001Z7AAAA0\", \"currencyCode\":\"INR\"}"}
        ); 
    ...
    // add the product to new quote
    //create payload of add 
    const quoteData = {
        record: {
            attributes: {
                type: "SBQQ__Quote__c",
                url: "/services/data/v63.0/sobjects/SBQQ__Quote__c/"+newquoteId
            },
            Name: ""+newquoteObj.record.Name,
            Id: ""+newquoteId
        },
        nextKey: 2,
        netTotal: 0.00,
    ...
    const quoteadderpayload = {
        context: JSON.stringify(contextData)
    };
    console.log('quoteData =>'+JSON.stringify(quoteadderpayload)); 
    
    
    // call to add
    var addProdtoQuote = await callSalesforceAPI(
        instanceUrl,
        accessToken,
        `/services/apexrest/SBQQ/ServiceRouter?loader=SBQQ.QuoteAPI.QuoteProductAdder`,
        'PATCH',
        quoteadderpayload
        ); 
    const prodaddeduqote =  JSON.parse(addProdtoQuote);
   
    console.log('Product Added string'+ addProdtoQuote);    
    console.log('Product Added'+ prodaddeduqote.lineItems);    
    //console.log('Product Added length '+addProdtoQuote.lineItems.length);
    //console.log('Product Added lineitems'+addProdtoQuote.lineItems);
    // Saving the quote 
    ...
    const modeldata = {
        ...
        nextKey:2,
        netTotal : 0.00,
        lineItems:prodaddeduqote.lineItems,
        lineItemGroups:[],
        customerTotal:0.00
    };
    const quoteSaverPayload = {
        saver : "SBQQ.QuoteAPI.QuoteSaver",
        model : JSON.stringify(modeldata)
    };

    console.log('quoteSaverPayload'+JSON.stringify(quoteSaverPayload));

    var saveQuote = await callSalesforceAPI(
        instanceUrl,
        accessToken,
    ...
    console.log('Quote Saved Successfully'+saveQuote);
